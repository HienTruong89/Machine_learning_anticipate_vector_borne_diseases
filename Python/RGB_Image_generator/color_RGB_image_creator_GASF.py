...
class TimeSeriesAnalyzer:
    ...
    def calculate_gasf(self, time_series):
        """
        ...
        matrix_gasf = gasf.fit_transform(time_series.reshape(1, -1))
        return matrix_gasf[0]
    ...
    def merge_gasf_matrices(self, *matrices):
        """
    ...
    def process_row(self, i, *dataframes):
        ...
        gasf_matrices = [self.calculate_gasf(row) for row in current_rows]

        # Merge the GASF matrices into a single RGB image
        merged_image = self.merge_gasf_matrices(*gasf_matrices)
