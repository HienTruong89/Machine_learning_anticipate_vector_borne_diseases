...
class TimeSeriesAnalyzer:
    ...
    def calculate_mtf(self, time_series):
        """
        ...
        matrix_mtf = mtf.fit_transform(time_series.reshape(1, -1))
        return matrix_mtf[0]
    ...
    def merge_mtf_matrices(self, *matrices):
        """
    ...
    def process_row(self, i, *dataframes):
        ...
        mtf_matrices = [self.calculate_mtf(row) for row in current_rows]

        # Merge the GASF matrices into a single RGB image
        merged_image = self.merge_mtf_matrices(*mtf_matrices)
