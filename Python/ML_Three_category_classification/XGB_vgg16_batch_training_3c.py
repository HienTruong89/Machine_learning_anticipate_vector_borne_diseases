...
IMAGE_FOLDER_PATH = "C:/Users/truon/OneDrive/onenote/Wageningen UR _ Research_Work/Project_July_Nov_2024/Image data_29_7_24/RGB"
LABELS_CSV_PATH = "C:/Users/truon/OneDrive/onenote/Wageningen UR _ Research_Work/Project_July_Nov_2024/Raw_data_29_7_24/Data_random_model_3classes/labels_3compartments.csv"
IMAGE_SIZE = (224, 224)
...
# Restructured code for XGBoost classifier with VGG16
...
def create_xgb_model(data, labels, n_estimators=300):
    """
    Creates and trains a Random Forest model using k-fold cross-validation.
    """
    ...
    for train_index,val_index in kfold.split(data,labels):
        ...
        model = xgb.XGBClassifier(objective='multi:softprob',max_depth=5,gamma=0.2,
                              eta=0.1,colsample_bytree=0.3,min_child_weight=1)
...
def create_and_evaluate_models_with_batches(folder_path, csv_path, VGG16_model, batch_size=500, epochs=5):
    ...
    for epoch in range(epochs):
        ...
        for batch_data, batch_labels in batch_loader:
            ...
            xgb_model = create_xgb_model(x_train_features, y_train)
            
            # Evaluate model on test data and accumulate predictions
            y_pred_batch, y_true_batch = accumulate_predictions_labels(xgb_model, x_test_features, y_test)
            all_y_pred.extend(y_pred_batch)  
            all_y_true.extend(y_true_batch)  
            
            train_results = evaluate_model(xgb_model, x_train_features, y_train)
            test_results = evaluate_model(xgb_model, x_test_features, y_test)
            
            y_probs = xgb_model.predict_proba(x_test_features)
        ...
        mean_accuracy, std_accuracy, mean_precision, mean_recall, mean_f1 = evaluate_model_mean_std([xgb_model], x_test_features, y_test)
