...
This study employed five different traditional machine learning methods as classifiers based on practical experience and general knowledge. The methods included  Logistic Regression (LR), Random Forest (RF), K-Nearest Neighbor (KNN), and Support Vector Machine (SVM) with both linear and radial basic functions (rbf), used to classify various scenarios of vector-borne diseases. Table 1 provides a comparative overview of these five machine-learning methods. 
Aspect	LR	RF	KNN	SVM (linear)	SVM (rbf)
Description	Estimate Probability 	Construct multiple decision trees	The majority class among K-nearest neighbors	Find the linear line by maximizing the margin 	Find the rbf hyperplane by maximizing the margin
Model Type	Linear	Ensemble (Tree-based)	Instance-based	Linear	Non-linear
Complexity	Low	Medium-High	Low	Medium	High
Interpretability	High	Medium - Low	High	Medium	Medium-Low
Training Speed	Fast	Medium	Slow	Medium	Slow
Memory Usage	Low	Medium - High	High	Low	Medium – High
Handling Non-Linearity	Poor	Good	Good	Poor	Excellent
Robustness	Medium	High	Low	High	High
Feature Importance	Yes	Yes	No	No	No
